...
${BUILD_DIR}/apps/build_disk_index --data_type float --dist_fn l2 --data_path data/sift/sift_learn.fbin --index_path_prefix ${INDEX_PATH_PREFIX} -R ${R_BUILD} -L${L_BUILD} -B ${B_BUILD} -M ${M_BUILD} -A ${A_BUILD} 
...
${BUILD_DIR}/apps/search_disk_index  --data_type float --dist_fn l2 --index_path_prefix ${INDEX_PATH_PREFIX} --query_file data/sift/sift_query.fbin  --gt_file data/sift/sift_query_learn_gt$K_GT -K ${K_SEARCH} -L ${L_SEARCH} --result_path ${RESULT_PATH} --num_nodes_to_cache 10000